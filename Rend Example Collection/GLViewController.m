...
- (void)viewDidLoad {
    ...
    glView_ = [[REGLView alloc] initWithFrame:[UIScreen mainScreen].bounds colorFormat:kEAGLColorFormatRGBA8 multisampling:YES];
    [self.view addSubview:glView_];
    
    camera_ = [[RECamera alloc] initWithProjection:kRECameraProjectionOrthographic];
    camera_.position = CC3VectorMake(0, 0, 100);
    camera_.upDirection = CC3VectorMake(0, 1, 0);
    camera_.lookDirection = CC3VectorMake(0, 0, -1);
    camera_.frustumNear = 1;
    camera_.frustumFar = 10000;
